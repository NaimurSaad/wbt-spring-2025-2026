...
/* ------------- AJAX search endpoint -------------
   ...
   ?page=ajax&type=pharmacist&q=...   (admin only)
   ?page=ajax&type=medicine&q=...        (pharmacist only)
*/
if ($page === 'ajax') {
    ...
    if ($type === 'pharmacist' && $_SESSION['user']['role'] === 'admin') {
        echo json_encode($q === '' ? getPharmacists($conn) : searchPharmacists($conn, $q));
    } elseif ($type === 'medicine' && $_SESSION['user']['role'] === 'pharmacist') {
...
if ($page === 'pharmacist' && $_SESSION['user']['role'] !== 'pharmacist')  { header('Location: index.php?page=login'); exit; }
...
switch ($page) {
    ...
    case 'pharmacist': pharmacistCtrl($conn); break;
    default:
        header('Location: index.php?page=login');
        exit;
}
