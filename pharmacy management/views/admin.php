...
<main class="main-content">
    <div class="page-header">
        <div>
            ...
            <p class="page-sub">Register, edit, search and remove pharmacist accounts</p>
        </div>
    </div>

    <?php if (isset($_GET['msg'])): ?>
        <?php $messages = ['added' => 'Pharmacist added successfully.',
                           'updated' => 'Pharmacist updated successfully.',
                           'deleted' => 'Pharmacist deleted successfully.'];
            ...
            <table class="data-table">
                ...
                <tbody id="tableBody">
                    <?php if (empty($pharmacists)): ?>
                        <tr><td colspan="5" class="empty">No pharmacists yet.</td></tr>
                    <?php else: ?>
                        <?php foreach ($pharmacists as $i => $pharmacist): ?>
                            <tr>
                                <td><?= $i + 1 ?></td>
                                <td><?= htmlspecialchars($pharmacist['name']) ?></td>
                                <td><?= htmlspecialchars($pharmacist['contact']) ?></td>
                                <td><?= htmlspecialchars($pharmacist['username']) ?></td>
                                <td class="text-right">
                                    <a class="btn-sm btn-edit"
                                       href="index.php?page=admin&action=edit&id=<?= $pharmacist['id'] ?>">Edit</a>
                                    <a class="btn-sm btn-delete"
                                       href="index.php?page=admin&action=delete&id=<?= $pharmacist['id'] ?>"
...
(function () {
    ...
    input.addEventListener('input', function () {
        ...
        timer = setTimeout(function () {
            fetch('index.php?page=ajax&type=pharmacist&q=' + encodeURIComponent(input.value.trim()),
